...
class Zeldovich_Recon:
    ...
    def __init__(
        ...
        self.pint = loginterp(k, p)(self.kint) * np.exp(
            -((self.kint / self.cutoff) ** 2)
        )
        ...
        self.sph = SphericalBesselTransform(
            self.qint,
            L=self.jn,
        ...
        self.sphx = SphericalBesselTransform(
            self.qint,
            L=self.jn,
            ncol=self.num_power_components_ds,
            threads=self.threads,
            import_wisdom=self.import_wisdom,
            wisdom_file=self.wisdom_file,
        )
        self.sphs = SphericalBesselTransform(
            self.qint,
            L=self.jn,
            ncol=1,
            threads=self.threads,
    ...
    def setup_2pts(self):
        ...
        # dd, ds, ss
        self.qfs["dd"] = QFuncFFT(
            self.kint,
            self.plins["dd"],
            pair_type="disp x disp",
            qv=self.qint,
            shear=self.shear,
        )
        self.qfs["ds"] = QFuncFFT(
            self.kint,
            self.plins["ds"],
            pair_type="disp x disp",
            qv=self.qint,
        ...
        # dm, sm
        self.qfs["dm"] = QFuncFFT(
            self.kint,
            self.plins["dm"],
            pair_type="disp x bias",
            qv=self.qint,
            shear=self.shear,
        )
        self.qfs["sm"] = QFuncFFT(
            self.kint,
            self.plins["sm"],
            pair_type="disp x bias",
            qv=self.qint,
        ...
        for pair in pairs:
            ...
            self.Xlins[pair] = (
                ...
                / 3
                * (
                    0.5 * self.qfs[a + a].xi0m2[0]
                    + 0.5 * self.qfs[b + b].xi0m2[0]
                    - self.qfs[pair].xi0m2
                    - self.qfs[pair].xi2m2
                )
            )
        ...
        for pair in pairs:
            ...
            if self.shear:
                J2 = 2.0 * self.qfs[pair].xi1m1 / 15 - 0.2 * self.qfs[pair].xi3m1
        ...
        if self.shear:
            self.zetas = {
                "mm": 2
                * (
                    4 * self.qfs["mm"].xi00 ** 2 / 45.0
                    + 8 * self.qfs["mm"].xi20 ** 2 / 63.0
                    + 8 * self.qfs["mm"].xi40 ** 2 / 35
                )
            }
            self.chis = {"mm": 4 * self.qfs["mm"].xi20 ** 2 / 3.0}
    ...
    def setup_rsd_facs(self, f, nu, D=1):
        ...
        self.nu = nu
        self.D = D
        self.Kfac = np.sqrt(1 + f * (2 + f) * nu**2)
        self.Kfac2 = self.Kfac**2
        self.s = f * nu * np.sqrt(1 - nu**2) / self.Kfac
        self.c = np.sqrt(1 - self.s**2)
        self.c2 = self.c**2
        self.ic2 = 1 / self.c2
        self.c3 = self.c**3
    ...
    def _G0_l_n(self, n, m, k):
        return (
            ...
            / gamma(n + 0.5)
            / gamma(1 - m + n)
            * hyp2f1(0.5 - n, -n, 0.5 - m - n, self.ic2)
        )
    ...
    def _dG0dA_l_n(self, n, m, k):
        ...
        ret = (
            fnm
            * (2 * n / self.c - 2 * n * self.c)
            * hyp2f1(0.5 - n, -n, 0.5 - m - n, x)
            / (k * self.qint)
        )
        if n > 0:
            ret += (
    ...
    def _d2G0dA2_l_n(self, n, m, k):
        ...
        ret = (
            ...
            * (1 - 1.0 / x)
            * (
                (2 * m - 1 - 4 * n * (m + 1)) * hyp2f1(0.5 - n, -n, 0.5 - m - n, x)
                + (1 - 4 * n**2 + m * (4 * n - 2))
                * hyp2f1(1.5 - n, -n, 0.5 - m - n, x)
            )
        )
        return ret
        ...
        if power == 0 or power == 1:
            for ll in range(lmax):
                ret += (
                    self.Hlfac**ll
                    * self._H0_l(ll, k)
                    * hyperu(-ll, n - ll + 1, -ksq * self.Bfac_ds)
                )
        elif power == 2:
            B = ksq * self.Bfac_ds
            for ll in range(lmax):
                ret += (
                    self.Hlfac**ll
                    * self._H0_l(ll, k)
                    * (hyperu(-ll, n - ll + 1, -B) + n / B * hyperu(-ll, n - ll, -B))
                )

        return (
            ret / self.sqrtpi
        )  # put the numerical factor here to avoid many evaluations
    ...
    def p_integrals(self, k, nmax=8):
        ...
        for l in range(self.jn):
            bias_integrands[0, :] = 1.0 * G0s[l]  # za
            bias_integrands[1, :] = -2 * K * self.Ulin * G1s[l]  # b1
            bias_integrands[2, :] = (
                self.corlin * G0s[l] - Ksq * self.Ulin**2 * G2s[l]
            )  # b1sq
            ...
            if l == 0:
                bias_integrands = bias_integrands * expon * (-2.0 / k / self.qint) ** l
    ...
    def make_ptable(self, f, nu, D=1, ks=None, kmin=1e-3, kmax=3, nk=100):
        ...
        if self.shear:
            ...
            self.Xs2 = self.Xs2s[pair] * D4
            self.sigmas2 = self.Xs2[-1]
    ...
    def pds_integrals_RecSym(self, k, nmax=8):
        ...
        for l in range(self.jn):
            ...
            if self.shear:
                bias_integrands[3, :] = (
                    -0.5 * Ksq * (self.Xs2 * G0s[l] + self.Ys2 * G2s[l])
                )  # bs
    ...
    def pds_integrals_RecIso(self, k, nmax=8):
        ...
        expon = np.exp(
            -0.5 * ksq * self.kaiser * (self.XYlin_ds0lag - self.sigma_ds0lag)
        )
        exponm1 = np.expm1(
            -0.5 * ksq * self.kaiser * (self.XYlin_ds0lag - self.sigma_ds0lag)
        )
        ...
        K1s = [
            0,
        ] + [k0 / self.kaiser / self.yq / k for k0 in K0s]
    ...
    def make_pdstable(
        self, f, nu, D=1, ks=None, kmin=1e-3, kmax=3, nk=100, method="RecSym"
    ):
        """
        Make a table of different terms of P(k) between a given
        'kmin', 'kmax' and for 'nk' equally spaced values in log10 of k
