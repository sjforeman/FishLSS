...
class SphericalBesselTransform:
    def __init__(
        ...
        self.ii_l = (
            self.Npad - self.Npad // 2
        )  # left and right indices sandwiching the padding
        ...
        self.fks = pyfftw.empty_aligned(
            (self.ncol, self.N // 2 + 1), dtype="complex128"
        )
        self.fs = pyfftw.empty_aligned((self.ncol, self.N), dtype="float64")

        pyfftw.config.NUM_THREADS = threads
        self.fft_object = pyfftw.FFTW(
            self.fs, self.fks, direction="FFTW_FORWARD", threads=threads
        )
        self.ifft_object = pyfftw.FFTW(
            self.fks, self.fs, direction="FFTW_BACKWARD", threads=threads
        )
        ...
        if low_ring:
            for ll in range(L):
                ...
                lnxy = (
                    self.Delta
                    / np.pi
                    * np.angle(self.UK(ll, q + 1j * np.pi / self.Delta))
                )  # ln(xmin*ymax)
                ys = np.exp(lnxy - self.Delta) * qs / (qs[0] * qs[-1])
                us = self.UK(ll, q + 2j * np.pi / self.N / self.Delta * ms) * np.exp(
                    -2j * np.pi * lnxy / self.N / self.Delta * ms
                )
    ...
    def sph(self, nu, fq):
        ...
        self.fs[
            :
        ] = 0  # on NERSC this seems necessary or this variable spills over from previous calls
        self.fs[
            :, self.Npad - self.Npad // 2 : self.N - self.Npad // 2
        ] = fq * self.q ** (3 - q)
