...
def loginterp(
    x, y, yint=None, side="both", lorder=9, rorder=9, lp=1, rp=-1, ldx=1e-6, rdx=1e-6
):
    ...
    if lneff < 0:
        ...
        print(
            "WARNING: Runaway index on left side, bad interpolation. Left index = %0.3e at %0.3e"
            % (lneff, x[l])
        )
    if rneff > 0:
        print("In function - ", inspect.getouterframes(inspect.currentframe())[2][3])
        print(
            "WARNING: Runaway index on right side, bad interpolation. Reft index = %0.3e at %0.3e"
            % (rneff, x[r])
        )

    xl = np.logspace(-12, np.log10(x[l]), int(10**5.0))
    xr = np.logspace(np.log10(x[r]), 12.0, int(10**5.0))
    yl = y[l] * (xl / x[l]) ** lneff
    yr = y[r] * (xr / x[r]) ** rneff
